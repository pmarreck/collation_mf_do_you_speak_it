...
export fn rcol_open(options: u32) callconv(.c) ?*Collator {
...
export fn rcol_strcoll(a: [*:0]const u8, b: [*:0]const u8) callconv(.c) i32 {
...
export fn rcol_strxfrm(dst: ?[*]u8, src: [*:0]const u8, n: usize) callconv(.c) usize {
...
test "ffi: get_sort_key memcmp order matches strcoll8" {
    ...
    const a = "apple";
    const b = "Apple";
    const na = rcol_get_sort_key(coll, a.ptr, a.len, &ka, ka.len);
    const nb = rcol_get_sort_key(coll, b.ptr, b.len, &kb, kb.len);
    try testing.expect(na > 0 and nb > 0);
    const cmp = rcol_strcoll8(coll, a.ptr, a.len, b.ptr, b.len);
